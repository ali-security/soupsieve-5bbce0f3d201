...
# Maximum number of selectors a single pattern is allowed to expand to.
# Selector lists, forgiving empty slots, custom selectors, and the precompiled
# selector lists that back complex pseudo-classes all count against this budget.
SELECTOR_LIMIT = 8192
...
# Quantifiers are never nested here: a `+` inside the repeated groups would let an
# unterminated value (`[a="xxx...`) trigger catastrophic backtracking.
VALUE = r'''
(?:"(?:\\(?:.|{nl})|[^\\"\r\n\f])*?"|'(?:\\(?:.|{nl})|[^\\'\r\n\f])*?'|{ident})
...
class CSSParser:
    ...
    def check_count(self) -> None:
        ...
        if self.count > SELECTOR_LIMIT:
            # Do not feed the pattern to `SelectorSyntaxError` as the offending
            # patterns are large and rendering their context is expensive.
            raise SelectorSyntaxError(
                'Selector exceeds pseudo-class nesting limit of {}'.format(SELECTOR_LIMIT)
            )
    ...
    def parse_combinator(
        ...
        if not has_selector:
            ...
            # If we are in a forgiving pseudo class, the empty slot is simply dropped,
            # but it still counts against the selector budget.
            if combinator == COMMA_COMBINATOR:
                self.count += 1
                self.check_count()
                del relations[:]
